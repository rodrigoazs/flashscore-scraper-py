...
def get_logo_id(src):
    if not src:
        return ""
    return src.split(".")[-2].split("-")[-1]


def download_logo(src):
    if not src:
        return
    folder = "logos"
    os.makedirs(folder, exist_ok=True)
    logo_id = get_logo_id(src)
    filename = os.path.join(folder, f"{logo_id}.png")
...
def get_archive_urls(html_content):
    ...
    for index, match in enumerate(matches):
        if index == 0:
            ...
            if not winner:
                update_urls.append(get_url_and_season(match))
                continue
            ...
            urls.append(get_url_and_season(match))
    return urls, update_urls


def extract_results_from_html(html_content, year, driver):
    soup = BeautifulSoup(html_content, "html.parser")
    results = [
        "timestamp,home_team,away_team,home_logo,away_logo,home_score,away_score,part,home_part,away_part,neutral"
    ]
    ...
    for match in matches[::-1]:
        ...
        download_logo(home_logo)
        download_logo(away_logo)
        team_logo_id = get_logo_id(home_logo)
        away_logo_id = get_logo_id(away_logo)
        results.append(
            ",".join(
                [
                    str(timestamp),
...
def extract_results(url, filename, year_str):
    try:
        ...
        results = extract_results_from_html(
            ...
            unidecode(div_element.get_attribute("innerHTML")),
            year,
            driver,
        )
...
if __name__ == "__main__":
    for folder, league_url in leagues.items():
        archive, update_archive = get_archive(league_url)
        os.makedirs(folder, exist_ok=True)
        ...
        for year_str, url in update_archive:
            ...
            if not os.path.exists(filename):
                ...
                merged_df = (
                    ...
                    .drop_duplicates(["timestamp", "home_team", "away_team"])
                )
                merged_df["home_part"] = merged_df["home_part"].astype("Int64")
                merged_df["away_part"] = merged_df["away_part"].astype("Int64")
